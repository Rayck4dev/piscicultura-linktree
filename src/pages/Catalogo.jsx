    ...
    <div className="relative flex flex-col min-h-screen bg-gradient-to-b from-blue-900/40 to-green-900/40 p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtrados.map((peixe, index) => (
          <div
            key={index}
            className="bg-black/40 backdrop-blur-md rounded-xl shadow-lg p-4 flex flex-col items-center"
          >
            {peixe.tipo === "img" && peixe.src ? (
              ...
              <div className="w-full mb-3">
                <video
                  ...
                  preload="none"
                />
                <p className="flex items-center justify-center gap-2 text-sm text-blue-800 mt-2 font-bold">
                  Toque no <FaPlay className="text-base" /> para assistir
                </p>
              </div>
            ) : (
              <div className="w-full h-64 flex items-center justify-center rounded-lg mb-3 bg-black/60 text-gray-300 font-bold">
                {statusLabels[peixe.status]}
              </div>
            )}

            <h2 className="text-2xl font-subtitle text-green-200 mb-2">
              {peixe.nome}
            </h2>
            ...
            <div className="flex gap-2 mt-2">
              <span
                className={`px-3 py-1 rounded-full text-xs font-bold ${statusColors[peixe.status]}`}
              >
                {statusLabels[peixe.status]}
              </span>
      ...
      <footer className="relative z-10 text-sm text-black font-sans text-center py-6 mt-8">
        ...
        <div className="max-w-xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-2 text-center mt-2">
          <a
            ...
            className="flex items-center gap-2 text-black font-subtitle font-bold text-sm sm:text-base z-10"
          >
